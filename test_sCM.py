...
import diffusers
from diffusers import AutoencoderKL, SCMScheduler
from diffusers.utils import check_min_version

from model.mmdit_flux import FluxTransformer2DModelTimestepX1GuidanceX1
from model.pipeline import FluxPipeline_sCM
...
def initialize_all_models(args, accelerator):
    ...
    transformer = FluxTransformer2DModelTimestepX1GuidanceX1.from_pretrained(
        ...
        for i in tqdm(range(len(prompts))):
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                res = pipeline(
                    ...
                    generator=generator,
                    intermediate_timesteps=1.3 if args.num_inference_steps==2 else None,
                )
            images.append(res[0])
...
def main(args):
    ...
    pipeline = FluxPipeline_sCM.from_pretrained(
        args.pretrained_model_name_or_path,
